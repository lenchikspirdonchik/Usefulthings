...
public class CalculatorActivity extends AppCompatActivity {
    ...
    private char opt = ' ';
    private boolean go = true, addWrite = true;
    ...
    public void onClick(View v) {
        ...
        final View.OnClickListener btnnum = new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        final View.OnClickListener btncalculat = new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Button calculatbtn = (Button) v;
                if (Pattern.matches("([-]?\\d+[.]\\d*)|(\\d+)", txt.getText().toString()))
                    if (go) {
                        val = calc(val, txt.getText().toString(), calculatbtn.getText().toString().charAt(1));
                        if (Pattern.matches("[-]?[\\d]+[.][0]*", String.valueOf(val))) {
                            txt.setText(String.valueOf((int) val));
                        } else {
                            txt.setText(String.valueOf(val));
                        }
                        opt=calculatbtn.getText().toString().charAt(1);
                        go = false;
                        addWrite = false;
                    } else {
                        opt=calculatbtn.getText().toString().charAt(1);
